...
function HeaderContent() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'glass border-b border-white/5 py-2 sm:py-3' : 'bg-transparent py-4 sm:py-5'}`}>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-72 sm:w-80 crystal-card rounded-xl overflow-hidden animate-float shadow-xl">
                    ...
                    <div className="max-h-64 sm:max-h-80 overflow-y-auto">
                      {notifications.map((notif) => (
                        <div key={notif.id} className={`p-3 sm:p-4 border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer ${notif.unread ? 'bg-purple-500/5' : ''}`}>
                          <div className="flex items-start gap-3">
                            <div className={`w-2 h-2 rounded-full mt-1.5 ${notif.unread ? 'bg-purple-500' : 'bg-transparent'}`} />
